...
* 20180312, 加入失联(被照顾)检测【正常：ON, 屏蔽：OFF】，校准虚拟化(container)流量统计异常　　　　　　
...
【服务端】：
```bash
wget https://raw.githubusercontent.com/CheungKwokHo/ServerStatus/master/autodeploy/config.json
docker run -d --restart=always --name=ServerStatus -v {$path}/config.json:/ServerStatus/server/config.json -p {$port}:80 -p {$port}:35601 adamcheung/ss

eg:
docker run -d --restart=always --name=ServerStatus -v ~/config.json:/ServerStatus/server/config.json -p 80:80 -p 35601:35601 adamcheung/ss
```
...
eg:
wget --no-check-certificate -qO client-linux.py 'https://raw.githubusercontent.com/CheungKwokHo/ServerStatus/master/clients/client-linux.py' && nohup python client-linux.py SERVER=0.0.0.0 USER=s04  >/dev/null 2>&1 &
```
...
【克隆代码】:
```
git clone https://github.com/CheungKwokHo/ServerStatus.git
```
